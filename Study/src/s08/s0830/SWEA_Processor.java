...
public class SWEA_Processor {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= T; t++) {
            ...
            for (int i = cores.size(); i >= 1; i--) {
                ...
                // 해당 조합에서 최대 코어수를 찾았다면 더 이상 탐색할 필요가 없음
                if (maxCores > 0) break;
            }
    ...
    static void check(int index, int connected, int wireLen, boolean[] selected) {
        // 모든 코어를 다 확인했을 때
        if (index == cores.size()) {
        ...
        // 선택되지 않은 core는 패스 
        if (!selected[index]) {
        	check(index + 1, connected, wireLen, selected);
            return;
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (canConnect) {
                ...
                // 다음 코어로 이동
                check(index + 1, connected + 1, wireLen + len, selected);
